...
//If a single share was bought on day P and sold on day Q, where 0 ≤ P ≤ Q < N, then the profit of such transaction is equal to 
//A[Q] − A[P], provided that A[Q] ≥ A[P]. Otherwise, the transaction brings loss of A[P] − A[Q].
//
...
//returns the maximum possible profit from one transaction during this period. The function should return 0 if it was impossible to
//gain any profit.
//
...
public class MaxProfit {
	...
	public int solution(int[] A) {
		...
		for(int i = 1 ; i < A.length ; i ++) {
			if(A[i] < minBuyPrice) minBuyPrice = A[i]; 
			else maxProfix = Math.max(maxProfix, A[i] - minBuyPrice); 
		}
