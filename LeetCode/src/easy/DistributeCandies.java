...
public class DistributeCandies {
	 ...
	 public int solution(int[] candyType) {
		 ...
		 HashSet<Integer> countCandy = new HashSet<>(); 
		 
		 for(int candy : candyType) {
			 if(!countCandy.contains(candy)) countCandy.add(candy); 
		 }
		 
		 int nCandy = candyType.length / 2; 
		 
		 return nCandy >= countCandy.size() ? countCandy.size() : nCandy ; 
	 }
	 
	 static public void main(String [] args) {
		 System.out.println(new DistributeCandies().solution(new int [] { 1,1,2,3,3,4 }));
	 }
