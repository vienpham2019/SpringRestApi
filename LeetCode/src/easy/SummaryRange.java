...
public class SummaryRange {
	...
	public List<String> solution(int[] nums){
		...
		for(int i = 0; i < nums.length; i ++) {
			if(i < nums.length - 1 && nums[i] + 1 != nums[i + 1] || i == nums.length - 1) {
				if(curr == nums[i]) result.add("" + curr); 
				else result.add(curr + "->" + nums[i]); 
				
				if(i < nums.length - 1) curr = nums[i + 1]; 
			}
		}
	...
	public static void main(String [] args) {
		System.out.println(new SummaryRange().solution(new int [] { 0,2,4,5,7,8,9,11 }));
	}
